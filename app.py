...
if st.button("Generate"):
    ...
    # (B) Generate the nomogram figure
    ...
    # Plot dashed reference lines
    for y_pos in y_positions[:-1]:
        ax.hlines(y=y_pos, xmin=0, xmax=100, color="black", linestyle="dashdot", linewidth=1.2)
    
    ax.hlines(y=0, xmin=0, xmax=400, color="black", linewidth=1.5)
    
    # Format axes
    ax.set_xticks(points_scale)
    ax.set_xticklabels(points_scale, fontsize=12, fontweight="bold")
    ...
    # Risk graph (sub-plot style)
    x_risk = np.linspace(0, 400, 100)
    y_risk = 1 / (1 + np.exp(-0.01 * (200 - x_risk)))
    ax_risk = ax.inset_axes([0.1, -0.4, 0.8, 0.2])
